...
> **专家蒸馏增量（2026-09-06 并入）**：涉及领域建模/架构模式选型时，同时读取 `references/expert-distill/software-architect-蒸馏.md`（架构通：领域发现四步法/模式选择矩阵/质量属性分析）；涉及后端数据 Schema/API 安全/可靠性/缓存设计时，读取 `references/expert-distill/backend-architect-蒸馏.md`（磐石石：Schema 铁律/API 安全基线/可靠性清单/缓存纪律）；涉及 MVP/快速验证项目的选型与灰度时，读取 `references/expert-distill/mvp-architect-蒸馏.md`（MVP 选型权重：扩展性低权重/团队熟悉度高权重 + 分场景栈表 + 搜索分级 + Feature Flag 轻量灰度）。各文档与原生方法论互补，只含增量不重复。
